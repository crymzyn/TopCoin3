...
int64 nChainStartTime = 1423736068;
...
const string strMessageMagic = "TopCoin3 Signed Message:\n";
...
void static PruneOrphanBlocks()
{
    ...
    map<uint256, CBlock*>::iterator it = mapOrphanBlocksByPrev.begin();
    while (pos--) it++;

    // As long as this block has other orphans depending on it, move to one of those successors.
    do {
        map<uint256, CBlock*>::iterator it2 = mapOrphanBlocksByPrev.find(it->second->GetHash());
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 24000000 * COIN; // 24 Million Premine coins
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TopCoin3
    bool fStrictPayToScriptHash = true; // Always active in TopCoin3
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TopCoin3", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "February 12, 2015. Topcoin Version 3 Has Been Created. 1423736068";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("08e5abe972b1ada1b1cbe1a5202e3d717369cdb9bbb5511cd18eb35049397702"));
