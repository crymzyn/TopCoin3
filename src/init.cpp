...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("TopCoin3 version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  TopCoin3d [options]                     " + "\n" +
                  "  TopCoin3d [options] <command> [params]  " + _("Send command to -server or TopCoin3d") + "\n" +
                  "  TopCoin3d [options] help                " + _("List commands") + "\n" +
                  "  TopCoin3d [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "TopCoin3:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("TopCoin3"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("TopCoin3"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: TopCoin3.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: TopCoin3d.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 22561 or testnet: 32561)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22562 or testnet: 32562)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  TopCoin3 is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "TopCoin3 server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("TopCoin3"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    #ifndef USE_LEVELDB
    #else
        filesystem::path blocksDir = GetDataDir() / "txleveldb";
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of TopCoin3") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart TopCoin3 to complete") << "\n";
